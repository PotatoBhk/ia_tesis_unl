...
class Yolo():
    ...
    def __init__(self, root = "./sources/yolo/"):        
        self.root = root        
        with open(self._coco_file, 'r') as f:
            self._classes = f.read().splitlines()

    def initModel(self):
        start_time = time.time()
        #Formatting paths 
        self._config_name = Utils.join_path(self.root, self._config_name)
        self._weights_name = Utils.join_path(self.root, self._weights_name)
        self._coco_file = Utils.join_path(self.root, self._coco_file)
